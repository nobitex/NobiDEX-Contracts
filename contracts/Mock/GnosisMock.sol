...
contract GnosisMock  {
    ...
    function proposeToUpdateSwapperModerator(address _swapper, address _newModerator)external{
              (bool success, bytes memory data) = _swapper.call(
            abi.encodeWithSignature("proposeToUpdateModerator(address)", _newModerator)
        );
        require(success, "ERROR: external call failed");
    ...
    function updateSwapperModerator(address _swapper, address[] memory _tokenAddresses)external{
              (bool success, bytes memory data) = _swapper.call(
